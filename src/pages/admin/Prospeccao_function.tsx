...
export function Prospeccao() {
  ...
  const handleFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        const errors: string[] = [];
        const valid: any[] = [];
        // Normalizar chaves — lowercase sem acento
        ...
        rows.forEach((row, i) => {
          const name = getField(row, "nome", "name", "Nome", "NOME").toString().trim();
          const rawPhone = getField(row, "telefone", "phone", "Telefone", "TELEFONE", "fone").toString();
          ...
          valid.push({
            ...
            interest: getField(row, "interesse", "interest", "Interesse").toString().trim() || null,
            source: getField(row, "origem", "source", "Origem").toString().trim() || null,
          });
        });
        setUploadData(valid);
  ...
  const handleUpload = async () => {
    ...
    setUploading(true);
    try {
      await supabase.from("prospect_leads").insert(uploadData.map(d => ({ ...d, status: "PROSPECTING", attempt_count: 0, current_step: 1, next_contact_at: new Date().toISOString(), script_id: uploadScriptId || null, instance_id: uploadInstanceId || null })));
      qc.invalidateQueries({ queryKey: ["prospect-leads"] });
      setUploadData([]); setUploadErrors([]);
      setUploadScriptId(""); setUploadInstanceId("");
    ...
    <div className="space-y-6">
      ...
      {view === "upload" && (
        <div className="space-y-5">
          ...
          {/* Seleção de Roteiro e Instância */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-black text-gray-400 uppercase tracking-widest mb-2 block">Roteiro de Prospecção *</label>
          ...
          {uploadData.length > 0 && (
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <Button onClick={handleUpload} disabled={uploading || !uploadScriptId || !uploadInstanceId} className="bg-emerald-600 hover:bg-emerald-500 font-bold disabled:opacity-40">
