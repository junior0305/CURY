...
function calcXPStats(totalXp: number): BrokerXP {
  ...
  const levelNames = ["", "Recruta", "Soldado", "Cabo", "Sargento", "Tenente", "Capitão", "Major", "Coronel", "General", "Lenda"];
  ...
  return { totalXp, level, levelName: levelNames[level], xpForNext, xpInCurrentLevel, progressPercent };
}
...
export function useGamification(brokerId?: string) {
  ...
  const loadXP = useCallback(async () => {
    ...
    const { data } = await supabase
      .from("broker_xp")
      .select("total_xp, level, level_name")
      .eq("broker_id", targetId)
      .maybeSingle();
    if (data) {
      setXpStats(calcXPStats(data.total_xp));
    } else {
      setXpStats(calcXPStats(0));
    }
  }, [targetId]);
  ...
  const loadMissions = useCallback(async () => {
    ...
    // Se não tem missões hoje, gerar 3 aleatórias
    if (!existing || existing.length === 0) {
      ...
      if (templates && templates.length > 0) {
        // Pegar 1 fácil, 1 médio, 1 difícil
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`xp-${targetId}`)
      .on("postgres_changes", { event: "*", schema: "public", table: "broker_xp", filter: `broker_id=eq.${targetId}` },
        () => loadXP())
      .on("postgres_changes", { event: "*", schema: "public", table: "daily_missions", filter: `broker_id=eq.${targetId}` },
        () => loadMissions())
      .subscribe();
    return () => { supabase.removeChannel(channel); };
  }, [targetId, loadXP, loadMissions]);
  ...
  const trackAction = useCallback(async (actionType: string) => {
    ...
    const matchingMissions = missions.filter(
      m => m.actionType === actionType && !m.completed && m.date === today
    );
    ...
    for (const mission of matchingMissions) {
      ...
      if (nowComplete) {
        ...
        // Criar prize claim se tiver prêmio real
  ...
  // Dar XP diretamente (ex: venda fechada)
