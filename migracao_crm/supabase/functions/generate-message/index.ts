...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Classificação usa Haiku — sem cache, sem knowledge
    if (classification) {
    ...
    // Geração usa Sonnet com prompt caching + base de conhecimento
    const knowledge = await getKnowledge(agent_id);
    const fullSystem = knowledge
        ...
        system: [
          {
            type: 'text',
            text: fullSystem,
            cache_control: { type: 'ephemeral' }  // Cache o system prompt + knowledge
          }
        ],
        messages: [
